...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  def mentions(keywords: List[String]): Boolean = keywords.exists((x: String) => text.contains(x))
}
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter((p: Tweet) => p.mentions(google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter((p: Tweet) => p.mentions(apple))
